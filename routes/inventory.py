...
# ========================================================
# 6. API PARA CARGAR RECETAS EN EL COTIZADOR
# ========================================================
@inventory_bp.route('/api/receta/<int:id>')
@login_required
def obtener_receta_api(id):
    """
    Esta función es llamada por el Javascript del Cotizador
    cuando seleccionas una receta para cargar.
    Devuelve el JSON con los materiales y maquinaria exactos.
    """
    conn = get_db()
    try:
        # 1. Obtener datos básicos del producto/receta
        prod = conn.execute("SELECT id, nombre FROM productos WHERE id=? AND user_id=?", (id, session['user_id'])).fetchone()
        if not prod:
            return jsonify({'error': 'Receta no encontrada'}), 404

        # 2. Obtener los Detalles de Materiales (¡ESTA ES LA CLAVE QUE FALTABA!)
        # Seleccionamos el ID del material y la cantidad guardada.
        ...
        # 4. Enviar todo el paquete como JSON al Javascript
        return jsonify({
            'id': prod['id'],
            'nombre': prod['nombre'],
            'materiales': materiales_lista, # Ahora sí lleva la lista llena
            'maquinaria': maquinaria_lista
        })

    except Exception as e:
        print(f"Error API Receta ID {id}: {e}")
        return jsonify({'error': 'Error al cargar los detalles de la receta'}), 500
    finally:
        conn.close()